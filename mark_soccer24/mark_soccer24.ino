...
//--------------------------------
int L, C, R;
int RefL = 2100, RefC = 2220, RefR = 2200;
//--------------------------------
POP32_Pixy2 pixy;
...
void setup() {
  //-----------------------
  //ShowADC();  //เมื่ออ่านค่าหาค่าเฉลี่ยเสร็จ ให้ comment ไม่ให้แสดง
  beep();
  //-----------------------
  pixy.init();
  reload();
  zeroYaw();
  //oled.mode(2);
  oled.text(4, 0, "Press SW_B zeroYaw");
  oled.text(5, 0, "Press SW_A Run....");
  oled.show();
  while (!SW_A()) {
    if (SW_B()) {
      zeroYaw();
    ...
    getIMU();
    oled.text(7, 0, "Yaw=%f     ", pvYaw);
    oled.show();
  }
}
void loop() {
  ConvertADC();  //อ่านค่า Sensor พื้นมาใช้

  if (C == 1) {
    holonomic(70, 270, 0); //ถอยหลัง
    delay(2000);
  } else if (L == 1) {  
    holonomic(70, 0, 0);   //ขยับไปทางขวา
    delay(2000);
  } else if (R == 1) {
    holonomic(70, 180, 0);  //ขยับไปทางซ้าย
    delay(2000);
  } else {
    Mission1();  //เรียกฟังก์ชั่น mission 
  }
