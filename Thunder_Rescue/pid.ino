...
void stdPID(int baseSpeed, float Kp, float Kd, int maxsp, int minsp) {
  ...
  int Error = Pos - 1500;  //3500 แปดเซนเซอร์
  ...
  if (LeftPower < -100) LeftPower = minsp;
  if (RightPower > 100) RightPower = maxsp;
  if (RightPower < -100) RightPower = minsp;
...
void SpinL() {
  ...
  while (1) {
    readCalibrate();
    if (F[1] <= REF[1]) {  //F[2] เช็คเซนเซอร์ซ้ายสุด
      MotorStop();
      Motor(-40, 40);
      delay(20);
      break;
...
void SpinR() {
  ...
  while (1) {
    readCalibrate();
    if (F[2] <= REF[2]) {  //F[5]  เช็คเซนเซอร์ขวาสุด
      MotorStop();
      Motor(40, -40);
      delay(20);
      break;
