...
#define numsensor 8
int last_value = 0;
int MaxF[numsensor] = { 894,	843,	717,	646,	760,	719,	716,	695 };  //MexF-100
int MinF[numsensor] = { 39,	35,	34,	33,	33,	33,	31,	34 };          //MinF+100
...
void view(int Sensor) {
  while (true) {
    if (Sensor == FRONTSENSOR) {
      //adc.begin(A0, A2, A1, A3);   //CLK, MOSI/DI, MISO/DO, CS
      //adc.begin(A0, A2, A1, 8);
      adc.begin(A3, A2, A1, A0);  //CS,MO,MI,CLK
      adc.begin(8, A2, A1, A0);
    } else if (Sensor == BACKSENSOR) {
      //adc.begin(A0, A2, A1, 8);    //CLK,MO,MI.CS  --> ADAFRUIT
      //adc.begin(A0, A2, A1, A3);
      adc.begin(8, A2, A1, A0);  //CS,MO,MI,CLK --> MCPXXXX.H
...
void Read(int Sensor) {
  int x = 0;
  if (Sensor == FRONTSENSOR) {
    //adc.begin(A0, A2, A1, A3);     //CLK,MO,MI.CS
    //adc.begin(A0, A2, A1, 8);
    adc.begin(A3, A2, A1, A0);  //CS,MO,MI,CLK
    adc.begin(8, A2, A1, A0);
